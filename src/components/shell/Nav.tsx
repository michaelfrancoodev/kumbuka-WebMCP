...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-[#0c0c0d]/90 backdrop-blur-xl border-t border-white/[0.06]">
        ...
        {items.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <NavLink key={to} to={to} className="flex flex-col items-center gap-1 px-4 py-1 relative transition-all">
              <Icon
                className={clsx('w-5 h-5 transition-colors', active ? 'text-white' : 'text-white/40')}
